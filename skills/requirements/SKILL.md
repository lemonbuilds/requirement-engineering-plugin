...
Arbeite standardmässig auf Deutsch, ausser der User wuenscht explizit eine
andere Sprache.
...
2. Die Benutzerinteraktion
   `[Betrachtungsgegenstand] muss/sollte/wird [wem?|was?] die Moeglichkeit bieten [Objekt] zu [Verb].`
3. Die Schnittstellenanforderung
   `[Betrachtungsgegenstand] muss/sollte/wird faehig sein [Objekt] zu [Verb].`
...
Nicht-funktionale Anforderungen beschreiben Qualitaetsmerkmale und muessen
messbar sein.
...
| NFR-002 | Beispiel | Das System muss während der Servicezeit eine Verfuegbarkeit von 99,9 Prozent pro Monat erreichen. | Availability | High | Open |

### Randbedingungen (RB)

Randbedingungen beschreiben Grenzen, Vorgaben oder Einschraenkungen der Loesung.
...
Siehe `REFERENCE.md` für ID-Praefixe, zulaessige Werte fuer `Priority` und
`Status` sowie die vorgesehenen NFR- und Randbedingungs-Kategorien. Verwende diese
Werte konsistent und unveraendert in den Tabellen.
...
|-------|-------|---------------------|----------------|
| Messbar | NFRs muessen eine Zahl oder einen Schwellenwert enthalten | System soll schnell sein | Seiten laden in unter 2 Sekunden |
| Singulaer | Eine Tabellenzeile enthaelt genau eine Anforderung | System soll Login und Export bieten | Zwei getrennte Anforderungen |
| Eindeutig | Keine subjektiven oder unklaren Begriffe | Benutzerfreundliche Oberflaeche | WCAG 2.1 AA fuer Kernablaeufe |
| Testbar | Es muss ein Pass/Fail-Test ableitbar sein | System ist zuverlaessig | 99,9 Prozent Verfuegbarkeit ueber 30 Tage |
| Eindeutige IDs | Keine doppelten IDs in irgendeiner Tabelle | Zwei Mal FR-001 | Jede ID wird genau einmal verwendet |

## Error Recovery

- Unvollständige Quelle: Liste fehlende Rollen, NFR-Kategorien oder Randbedingungen auf und bitte nur dann um Klaerung, wenn eine belastbare Ausarbeitung sonst nicht moeglich ist
- Mehrdeutige Anforderung: Formuliere sie in eine messbare Variante um und markiere den Schwellenwert als Annahme, falls keine Rueckfrage möglich ist
- Widerspruechliche Anforderungen: Weise explizit auf den Konflikt hin, zum Beispiel zwischen Echtzeit-Anforderung und Batch-Randbedingung
- Fehlende Stakeholder-Rollen: Verwende vorlaeufig generische Rollen wie Benutzer, Administrator und System und kennzeichne sie zur spaeteren Validierung
...
7. Pruefe alle Anforderungen gegen die Quality Checks
...
## Standardstruktur fuer `docs/requirements.md`

Das Ergebnis soll standardmaessig diese Abschnitte enthalten:
...
- Wenn der User ausdruecklich User Stories, NFRs, Risiken oder offene Fragen verlangt, nimm diese immer auf
...
- Bevorzuge klare deutsche Begriffe; uebernimm englische Labels nur, wenn sie projektweit etabliert sind
...
- `Erstelle ein PRD`
- `Sammle Anforderungen fuer dieses Feature`
- `Dokumentiere die Feature-Spezifikation`
- `Formuliere Anforderungn gemäß der Satzschablonen`
- `Definiere NFRs`
